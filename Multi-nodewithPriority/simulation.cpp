...
int q_size_array[link_size] = {sys_1_size - 1, sys_2_size - 1, sys_2_size - 1, sys_2_size - 1, sys_2_size - 1};
...
unsigned int arr_type, last_stat;
...
struct node
{
	...
	d_high_low arr_num, num_in_q, sum_in_q, num_in_sys, sum_in_sys, total_block, num_dep, num_served;
	f_high_low sum_q_time, sum_sys_time, time_ave_sum_in_sys, time_ave_sum_in_q;
	customData custom[51];
...
void arr(node &q1, int priority)
{
	...
	else if (q1.num_in_q.total < q1.q_size)
	{
		...
		for (int i = q1.num_in_q.total; i > 1; i--)
		{
			if (q1.custom[i].prior_lvl < q1.custom[i - 1].prior_lvl)
			{
				customData tmp;
				tmp = q1.custom[i - 1];
				q1.custom[i - 1] = q1.custom[i];
				q1.custom[i] = tmp;
			}
			else
				break;
...
void relay_dep(node &q1)
{
	...
	if (q1.mmoo_stat == 1 && (q1.custom[0].arr_type == 0 || q1.custom[0].arr_type == 1))
	{
		q2->num_arr_l0++;
		if (last_stat == 2)
		{
	...
	else if (q1.mmoo_stat == 2 && (q1.custom[0].arr_type == 0 || q1.custom[0].arr_type == 1))
	{
		q2->num_arr_l1++;
		if (last_stat == 1)
		{
	...
	else if (q2->num_in_q.total < q2->q_size)
	{
		...
		for (int i = q2->num_in_q.total; i > 1; i--)
		{
			if (q2->custom[i].prior_lvl < q2->custom[i - 1].prior_lvl)
			{
				customData tmp;
				tmp = q2->custom[i - 1];
				q2->custom[i - 1] = q2->custom[i];
				q2->custom[i] = tmp;
			}
			else
				break;
...
{
	now = current_event_time;
	if (mmoo_arr_rate == arr_l0)
		arr_type = 0;
	else if (mmoo_arr_rate == arr_l1)
		arr_type = 1;
	arr(q1, 2);
...
void print_metrics()
{
	...
	f_high_low th_total, pb_total, pb_total_numer, L_total, Lq_total, W_total, Wq_total;
	for (int i = 0; i < link_size; i++)
	{
		printf("============================\n");
		...
		th_total.total += th[i].total;
		th_total.high += th[i].high;
		th_total.low += th[i].low;

		pb_total_numer.total += q[i].total_block.total;
		pb_total_numer.high += q[i].total_block.high;
		pb_total_numer.low += q[i].total_block.low;

		L_total.total += L[i].total;
		L_total.high += L[i].high;
		L_total.low += L[i].low;

		Lq_total.total += Lq[i].total;
		Lq_total.high += Lq[i].high;
		Lq_total.low += Lq[i].low;
	...
	/*
		W_total.total=W[1].total;
		W_total.high=W[1].high;
		W_total.low=W[1].low;

		Wq_total.total=Wq[1].total;
		Wq_total.high=Wq[1].high;
		Wq_total.low=Wq[1].low;
	*/
	pb_total.total = pb_total_numer.total / q[0].arr_num.total;
	pb_total.high = pb_total_numer.high / q[0].arr_num.high;
	pb_total.low = pb_total_numer.low / q[0].arr_num.low;
